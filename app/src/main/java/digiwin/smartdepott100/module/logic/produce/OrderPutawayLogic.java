 ...
 * @author maoheng
 * @des 工单入库上架
 * @date 2017/9/07 10:58
 */
public class OrderPutawayLogic extends CommonLogic {

    public static OrderPutawayLogic logic;

    private OrderPutawayLogic(Context context, String module, String timestamp) {
        super(context, module, timestamp);
    }

    public static OrderPutawayLogic getInstance(Context context, String module, String timestamp) {

        return logic = new OrderPutawayLogic(context, module, timestamp);
    }
        ...
        ThreadPoolManager.getInstance().executeTask(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    String createJson = JsonReqForERP.objCreateJson(mModule, "als.b017.list.get", mTimestamp, filterBean);
    ...
    public void getSuitPickingSum(final ClickItemPutBean clickItemPutBean, final GetZSumListener listener) {
        ThreadPoolManager.getInstance().executeTask(new Runnable() {
            ...
            public void run() {
                try {
                    String createJson = JsonReqForERP.objCreateJson(mModule, "als.b017.list.detail.get", mTimestamp, clickItemPutBean);
                    OkhttpRequest.getInstance(mContext).post(createJson, new IRequestCallbackImp() {
                        @Override
                        public void onResponse(String string) {
                            String error = mContext.getString(R.string.unknow_error);
    ...
    public void commit(final Map<String, String> map, final CommitListener listener) {
        ThreadPoolManager.getInstance().executeTask(new Runnable() {
            ...
            public void run() {
                try {
                    String createJson = JsonReqForERP.mapCreateJson(mModule, "als.b017.submit", mTimestamp, map);
                    OkhttpRequest.getInstance(mContext).post(createJson, new IRequestCallbackImp() {
                        @Override
                        public void onResponse(String string) {
                            String error = mContext.getString(R.string.unknow_error);
    ...
    public void docNoFIFO(final Map<String, String> map, final PostMaterialFIFOListener listener) {
        ThreadPoolManager.getInstance().executeTask(new Runnable() {
            @Override
            public void run() {
                String json = JsonReqForERP.mapCreateJson(mModule, "als.b017.fifo.get", mTimestamp, map);
