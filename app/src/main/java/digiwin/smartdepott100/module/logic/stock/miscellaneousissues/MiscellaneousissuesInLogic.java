...
public class MiscellaneousissuesInLogic extends CommonLogic {
    private static MiscellaneousissuesInLogic logic;
    private String TAG = "MiscellaneousissuesInLogic";

    protected MiscellaneousissuesInLogic(Context context, String module, String timestamp) {
        super(context, module, timestamp);
    }

    public static MiscellaneousissuesInLogic getInstance(Context context, String module, String timestamp) {
        logic = new MiscellaneousissuesInLogic(context, module, timestamp);
        return logic;
    }
    ...
    public void getMIISumData(final Map<String, String> map, final GetZSumListener listener) {
        ThreadPoolManager.getInstance().executeTask(new Runnable() {
            ...
            public void run() {
                try {
                    String createJson = JsonReqForERP.mapCreateJson(mModule, "als.c008.list.detail.get", mTimestamp, map);
                    OkhttpRequest.getInstance(mContext).post(createJson, new IRequestCallbackImp() {
                        @Override
     ...
     * @param map map可以直接为空
     */
    public void commitMIIData(final Map<String, String> map, final CommitListener listener) {
        ThreadPoolManager.getInstance().executeTask(new Runnable() {
            @Override
            public void run() {
                try {
                    String createJson = JsonReqForERP.mapCreateJson(mModule, "als.c008.submit", mTimestamp, map);
                    OkhttpRequest.getInstance(mContext).post(createJson, new IRequestCallbackImp() {
                        @Override
                        public void onResponse(String string) {
                            String error = mContext.getString(R.string.unknow_error);
