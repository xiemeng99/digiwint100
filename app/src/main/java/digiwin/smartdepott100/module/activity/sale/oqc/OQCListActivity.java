 ...
 * @author xiemeng
 * @des
 * @date 2017/10/3 11:03
 */

public class OQCListActivity extends BaseTitleHActivity {
    ...
    private OQCListAdapter adapter;
    ...
    private Handler.Callback mCallback = new Handler.Callback() {
        ...
        public boolean handleMessage(Message message) {
            switch (message.what) {
                case GETLISTWHAT:
                    OQCReqBean oqcReqBean = new OQCReqBean();
                    oqcReqBean.setSource_no(etDeliveryNoteNo.getText().toString().trim());
                    oqcReqBean.setRefer_no(etPurchaseOrder.getText().toString().trim());
                    oqcReqBean.setBarcode_no(etMaterialNumber.getText().toString().trim());
                    oqcReqBean.setCustomer_no(etSupplier.getText().toString().trim());
                    oqcReqBean.setDate_begin(startDate);
                    oqcReqBean.setDate_end(endDate);
                    qcList.clear();
                    adapter = new OQCListAdapter(activity, qcList);
                    ryList.setAdapter(adapter);
                    showLoadingDialog();
                    logic.getFQCScanDatas(oqcReqBean, new OQCInspectLogic.GetScanListener() {
                        @Override
    ...
    private void itemClick(QCScanData qcScanData) {
        ...
        ActivityManagerUtils.startActivityBundleForResult(activity, OQCItemActivity.class, bundle, REQUESTCODE);
    }
    ...
    protected int bindLayoutId() {
        return R.layout.activity_oqc_list;
    }
